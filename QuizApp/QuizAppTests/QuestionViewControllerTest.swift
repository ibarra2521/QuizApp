...
class QuestionViewControllerTest: XCTestCase {
    ...
    func test_viewDidLoad_rendersOptionsText() {
        //sut.tableView.dataSource?.tableView(sut.tableView, cellForRowAt: indexPath)
        XCTAssertEqual(makeSUT(options: ["A1", "A2"]).tableView.title(at: 0), "A1")
        XCTAssertEqual(makeSUT(options: ["A1", "A2"]).tableView.title(at: 1), "A2")
    }

    func test_optionSelected_withSingleSelection_notifiesDelegateWithLastSeletion() {
    ...
    func test_optionSelected_withMultipleSelectionEnable_notifiesDelegateSeletion() {
        var receivedAnswer = [String]()
        let sut = makeSUT(options: ["A1", "A2"]) { receivedAnswer = $0 }
